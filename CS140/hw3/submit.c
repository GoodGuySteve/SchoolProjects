...
void readnbody(double** s, double** v, double* m, int n) {
	...
	//myrank = 0;
	//nprocs = 1;
	double* sendbuf = (double*)malloc(sizeof(double) * n * 7);
	double* recbuf = (double*)malloc(sizeof(double) * n / nprocs * 7);
	...
	MPI_Scatter(sendbuf, n*7/nprocs, MPI_DOUBLE, recbuf, n*7/nprocs, MPI_DOUBLE, 0, MPI_COMM_WORLD); 
	...
	for (j = 0; j < nprocs; ++j) {
		if (myrank == j) {
			//fprintf(stderr, "Printing results for proc %d:\n", myrank);
			for (i = 0; i < n / nprocs; i++) {
				fprintf(stderr, OUTPUT_BODY, s[i][0], s[i][1], s[i][2], v[i][0], v[i][1], v[i][2], m[i]);
			}
		}
		//fprintf(stderr, "%d calling barrier\n", myrank);
		MPI_Barrier(MPI_COMM_WORLD);
	}*/	
}
	...
	for (itNum = 0; itNum < iter; ++itNum) {
		...
		for (procStep = 0; procStep < nprocs - 1; ++procStep) { //for each send and receive round
			...
			if (myrank % 2 == 0) {
				...
				MPI_Send(sendBuf, size * 7, MPI_DOUBLE, (myrank + 1) % nprocs, 0, MPI_COMM_WORLD);
				//fprintf(stderr, "Send successful.\n");
				MPI_Recv(recBuf, size * 7, MPI_DOUBLE, (nprocs + myrank - 1) % nprocs, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			...
			for (k = 0; k < nprocs; ++k) {
				if (myrank == k) {
					fprintf(stderr, "Printing results for proc %d receive:\n", myrank);
					for (i = 0; i < n / nprocs; i++) {
						fprintf(stderr, OUTPUT_BODY, recBuf[7*i+1], recBuf[7*i+2], recBuf[7*i+3], recBuf[7*i+4], recBuf[7*i+5], recBuf[7*i+6], recBuf[7*i]);
					}
				}
				//fprintf(stderr, "%d calling barrier\n", myrank);
				MPI_Barrier(MPI_COMM_WORLD);
		...
		for (j = 0; j < nprocs; ++j) {
			if (myrank == j) {
				fprintf(stderr, "Printing results for proc %d:\n", myrank);
				for (i = 0; i < n / nprocs; i++) {
					fprintf(stderr, OUTPUT_BODY, axyz[i][0], axyz[i][1], axyz[i][2], v[i][0], v[i][1], v[i][2], m[i]);
				}
			}
			//fprintf(stderr, "%d calling barrier\n", myrank);
			MPI_Barrier(MPI_COMM_WORLD);
