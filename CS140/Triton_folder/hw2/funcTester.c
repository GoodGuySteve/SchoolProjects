...
int main(int argc, char **argv)
{
	...
	vec_temp = (double *)malloc(size*sizeof(double));
	...
	//DEBUG
	int i;
	printf("\nProcess %d has generated the following matrix:\n",myrank);
	for (i = 0; i < size * size / nprocs; ++i) printf("%f ", mat[i]);
	printf("\n");
	
	//Generate a random vector
	generatevec(vec,size); 
	printf("\nProcess %d has generated the following vector:\n",myrank);
	for (i = 0; i < size / nprocs; ++i) printf("%f ", vec[i]);
	printf("\n");

	//DEBUG test matvec
	matVec(mat, vec, vec_temp, size, size);
	printf("\nProcess %d multiplication gives the following vector:\n",myrank);
	for (i = 0; i < size / nprocs; ++i) printf("%f ", vec_temp[i]);
	printf("\n");
