...
int nworkers = __cilkrts_get_nworkers(void);
...
void bfs (int s, graph *G, int **levelp, int *nlevelsp, 
  ...
  while (levelsize[thislevel] > 0) {
    ...
    for (int i = 0; i < levelsize[thislevel]; i++) {
      v = curLevelV[i];//queue[front++];       // v is the current vertex to explore from
    ...
    cilk_for (int e = G->firstnbr[v]; e < G->firstnbr[v+1]; e++) {
        ...
        if (level[w] == -1) {   // w has not already been reached
          ...
          nextLevelV.insert(w);//queue[back++] = w;    // put w on queue to explore
        }
      }
    }
    ...
    //DEBUG
    cout << "Level "<< thislevel << " has stored " << nextLevelV.size() << " elements" << endl;

    delete[] curLevelV;
    curLevelV = nextLevelV.toArray();
    nextLevelV.init(nextLevelV.scale);	
  }
  *nlevelsp = thislevel;
  //free(queue);
}
