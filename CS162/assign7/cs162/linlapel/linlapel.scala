  ...
  def eval(goals: Seq[Stacker], _Δ: Seq[Assume], eq: Equiv): Unit =
    if (goals nonEmpty) goals.head match {
      ...
      case goalClo @ Closure(_: Predicate, _, _Γ) ⇒ {
		...
		_Δ.foldLeft(0)((index, a) => {
			a match {
				case Fact(vars, pred) => {
					val newClo = Closure(pred, (vars map {_ -> LogicV()}).toMap, _Γ)
					matchPredicates(newClo, goalClo, eq) match {
						case Some(newEq) => eval(goals.tail, _Δ.splitAt(index)._1 ++ _Δ.splitAt(index)._2.tail, newEq)
						case None => {}
					}
				}
				case Clause(vars, hd, body) => {
					...
					matchPredicates(newClo, goalClo, eq) match {
						case Some(newEq) => eval(Closure(body, newVMap, _Γ) +: goals.tail, _Δ.splitAt(index)._1 ++ _Δ.splitAt(index)._2.tail, newEq)
						case None => {}
					}
				}
